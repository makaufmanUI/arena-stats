...
if st.session_state.arena_data is not None:
    big_spacer()
    st.write(st.session_state.arena_data[::-1])
#     st.write(st.session_state.arena_data.reverse())
    ...
    st.markdown("### 3v3")
    small_spacer()
    st.pyplot(plot_rating(get_3v3_matches(st.session_state.arena_data), showEnemy=True))
...
#     for comp in comps:
#         games_played[comp] = 0
#     for match in data_3v3:
#         games_played[match['comp']] += 1
    ...
    for comp in winrates:
        ...
        with col1:
            st.write(  f"{', '.join(classes)} - {winrates[comp]*100:.1f}%" + f" ({games} game(s))"  )
